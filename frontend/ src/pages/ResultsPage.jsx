...
const ResultsPage = ({ result, image, reset }) => (
  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 animate-fade-in">
    <div className="grid md:grid-cols-2 gap-8">
      <img src={image} alt="Classified art" className="w-full h-auto object-contain rounded-lg shadow-md max-h-[500px]" />
      <div className="flex flex-col justify-center">
        <div className="mb-4">
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{result?.style || 'Unknown'}</p>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
            <div className="bg-blue-500 h-2.5 rounded-full" style={{ width: `${(result?.confidence || 0) * 100}%` }} />
          </div>
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-300 leading-relaxed">{result?.description || 'No description available.'}</p>
        </div>

        <div className="mt-auto pt-4">
          <ActionButton onClick={reset} icon={<Upload />} text="Try Another Image" primary />
        </div>
      </div>
    </div>
  </div>
